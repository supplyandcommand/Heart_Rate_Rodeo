...
surrogate_RESP <- function(data, condition_name) {
  ...
  for (n in 1:153) {
    # participants
    p1 <- dplyr::filter(data, data$greatness == expand[n,1] , data$condition == condition_name) 
    p2 <- dplyr::filter(data, data$greatness == expand[n,2] , data$condition == condition_name)
