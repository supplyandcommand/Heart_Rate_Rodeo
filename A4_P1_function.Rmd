...
## THE FUNCTION
optimizeAlot_discount <- function(df, group_nr, parameter_list) {
  ...
  if (is.character(HR) & is.character(RESP)) {
    bind <- data.frame("radius" = c(NA, NA),
                       "delay" = c(NA, NA),
                       "emddim" = c(NA, NA)) %>%
      cbind.data.frame('group' = group_nr) %>%
      cbind.data.frame("metric" = metric)
  } 
  
  else {
    bind <- rbind.data.frame(HR, RESP) %>%
      cbind.data.frame('group' = group_nr) %>%
      cbind.data.frame("metric" = metric)
  }
...
crqa_hell <- function(df, group_nr, var1, var2) {
  ...
  # computing
  crqa_list <- list(crqa(ts1 = data[[var1]], 
                            ts2 = data[[var2]], 
