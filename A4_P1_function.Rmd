...
```{r crqa function}
# CRQA LOOP
crqa_hell_RESP <- function(df, group_nr) {
  ...
  # computing
  for (i in 7:9) {
    crqa_list[i] <- list(crqa(ts1 = data[["resp1S"]], 
                           ts2 = data[["resp2S"]], 
  ...
  metrics_conditions <- as.matrix(med_optimal_para[1:3,1:2])
  group_nr_matrix <- as.matrix(rep(group_nr, times = 3))
...
crqa_hell_HR <- function(df, group_nr) {
  ...
  # computing
  for (i in 4:6) {
    crqa_list[i] <- list(crqa(ts1 = data[["hr1S"]], 
                              ts2 = data[["hr2S"]], 
                              delay = as.numeric(med_optimal_para[i, 3]), #delay 
                              embed = as.numeric(med_optimal_para[i, 4]), #emdim 
                              radius = as.numeric(med_optimal_para[i, 5]), #radius
...
crqa_hell_ECG <- function(df, group_nr) {
  ...
  # computing
  for (i in 1:3) {
    crqa_list[i] <- list(crqa(ts1 = data[["ecg1S"]], 
                              ts2 = data[["ecg2S"]], 
                              delay = as.numeric(med_optimal_para[i, 3]), #delay 
                              embed = as.numeric(med_optimal_para[i, 4]), #emdim 
                              radius = as.numeric(med_optimal_para[i, 5]), #radius
...
# running the CRQA functions
takes like an hour to run!!
```{r crqa run}
set.seed(666)
start = Sys.time()
