...
# CRQA LOOP
## HR
crqa_1 <- function(df, group_nr, cond, var1, var2, del, emb, rad) {
  ...
  # computing
  crqa_list <- list(crqa(ts1 = data[["hr1S"]], 
                            ts2 = data[["hr2S"]], 
                            delay = del, #delay 
                            embed = emb, #emdim 
                            radius = rad, #radius
  ...
  #metrics_conditions <- as.matrix(med_optimal_para[1,1])
  #group_nr_matrix <- as.matrix(group_nr)
...
## RESP
crqa_RESP <- function(df, group_nr) {
  
  data <- df %>%
    dplyr::filter(group == as.character(group_nr))
  
  # computing
  crqa_list <- list(crqa(ts1 = data[["resp1S"]], 
                            ts2 = data[["resp2S"]], 
                            delay = as.numeric(med_optimal_para[2, 2]), #delay 
                            embed = as.numeric(med_optimal_para[2, 3]), #emdim 
                            radius = as.numeric(med_optimal_para[2, 4]), #radius
                            normalize=0,
                            rescale=0,
                            mindiagline = 2,
                            minvertline = 2))
  ...
  # metrics, conditions, group nr
  metrics_conditions <- as.matrix(med_optimal_para[2,1])
  group_nr_matrix <- as.matrix(group_nr)
...
```{r}
d4 %>% group_by(group, condition) %>% tally()
d4_10_conv[2401, ]$condition
d4 <- filter(d4, ecg2S != d4_10_conv[2401, ]$ecg2S)
```
